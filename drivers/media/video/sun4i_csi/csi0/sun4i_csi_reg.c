...
/* buffer */
void inline bsp_csi_set_buffer_address(struct csi_dev *dev,
		__csi_buf_t buf, u32 addr) {
	//bufer0a +4 = buffer0b, bufer0a +8 = buffer1a
	W(dev->regs+CSI_REG_BUF_0_A + (buf<<2), addr);
}

u32 inline bsp_csi_get_buffer_address(struct csi_dev *dev,
		__csi_buf_t buf) {
	u32 t;
...
void inline bsp_csi_double_buffer_select_next(struct csi_dev *dev,
		__csi_double_buf_t type) {
	if (CSI_BUF_A == type) {
...
void inline bsp_csi_double_buffer_get_status(struct csi_dev *dev,
		__csi_double_buf_status_t * status) {
	u32 t;
...
void bsp_csi_capture_get_status(struct csi_dev *dev,
...
/* size */
void bsp_csi_set_size(struct csi_dev *dev, u32 length_h, u32 length_v,
		u32 buf_length_h) {
...
void inline bsp_csi_int_get_status(struct csi_dev *dev,
...
void inline bsp_csi_int_clear_status(struct csi_dev *dev,
		__csi_int_t interrupt) {
